...
const fragmentShader = `
  ...
  vec4 cppn_fn(vec2 coordinate, float in0, float in1, float in2) {
    ...
    * buf[7]
    ...
    // Remap CPPN output to Ember orange palette
    float intensity = dot(buf[0].xyz, vec3(0.299, 0.587, 0.114));
    vec3 bgColor = vec3(0.039, 0.039, 0.039);       // #0A0A0A
    vec3 emberPrimary = vec3(1.0, 0.42, 0.21);       // #FF6B35
    vec3 emberLight = vec3(1.0, 0.56, 0.4);          // #FF8F66
    vec3 emberColor = mix(bgColor, emberPrimary, pow(intensity, 0.8));
    emberColor = mix(emberColor, emberLight, smoothstep(0.5, 0.9, intensity) * 0.4);
    return vec4(emberColor, 1.0);
  }
...
function ShaderBackground() {
    ...
    return (
        <div ref={canvasRef} className="bg-[#0A0A0A] absolute inset-0 z-0 w-full h-full opacity-0" aria-hidden>
            <Canvas
                camera={camera}
            ...
            <div className="pointer-events-none absolute inset-0 bg-gradient-to-t from-[#0A0A0A]/40 via-transparent to-[#0A0A0A]/20" />
        </div>
    );
}
...
function CountUpMetric({ value, suffix, label }: { value: number; suffix: string; label: string }) {
            ...
            (entries) => {
                if (entries[0].isIntersecting && !hasAnimated.current) {
                    ...
                    gsap.to(obj, {
                        ...
                        duration: 2,
                        ease: 'power2.out',
                        onUpdate: () => setDisplayed(Math.round(obj.val * 10) / 10),
                    });
                }
            },
            { threshold: 0.5 }
    ...
    const formatted = suffix === 'M+' ? `R$${displayed}${suffix}` : `${displayed}${suffix}`;

    return (
        <div className="flex flex-col items-center text-center">
            <span ref={ref} className="font-heading font-bold text-[32px] text-[#FF6B35]">
                {formatted}
            </span>
            <span className="text-[12px] text-[#757575] uppercase tracking-[1px] mt-1 font-body font-semibold">
                {label}
            </span>
        </div>
    );
        ...
        () => {
            ...
            document.fonts.ready.then(() => {
                ...
                if (metricsRef.current) {
                    gsap.set(metricsRef.current, { autoAlpha: 0, y: 12 });
                }
                ...
                if (metricsRef.current) {
                    tl.to(metricsRef.current, { autoAlpha: 1, y: 0, duration: 0.6 }, '-=0.2');
                }
            });
        },
        { scope: sectionRef },
    ...
    return (
        <section ref={sectionRef} className="relative h-screen w-screen overflow-hidden bg-[#0A0A0A]">
            ...
            <div className="relative z-10 mx-auto flex h-full max-w-7xl flex-col items-center justify-center gap-4 px-6 text-center">
                ...
                <p ref={subtitleRef} className="text-[16px] md:text-[20px] font-light font-body tracking-[1px] text-[#BDBDBD]">
                    Analista de Dados e Operações
                </p>

                <div ref={ctaRef} className="flex flex-wrap items-center justify-center gap-4 pt-4">
                    <a
                        href="https://linkedin.com/in/isaque-maia-8ab346268"
                        target="_blank"
                        rel="noopener noreferrer"
                        className="group inline-flex items-center gap-2 rounded-2xl border border-[rgba(255,107,53,0.4)] bg-[rgba(255,107,53,0.1)] px-6 py-3 text-sm font-semibold tracking-tight text-[#FF6B35] backdrop-blur-sm transition-all duration-300 hover:bg-[rgba(255,107,53,0.2)] hover:border-[rgba(255,107,53,0.7)] focus:outline-none focus:ring-2 focus:ring-[#FF6B35]/30"
                    >
                        <Linkedin size={18} />
                        LinkedIn
                    </a>
                    <a
                        href="https://github.com/isaquecmaia"
                        target="_blank"
                        rel="noopener noreferrer"
                        className="group inline-flex items-center gap-2 rounded-2xl border border-[rgba(255,107,53,0.4)] bg-[rgba(255,107,53,0.1)] px-6 py-3 text-sm font-semibold tracking-tight text-[#FF6B35] backdrop-blur-sm transition-all duration-300 hover:bg-[rgba(255,107,53,0.2)] hover:border-[rgba(255,107,53,0.7)] focus:outline-none focus:ring-2 focus:ring-[#FF6B35]/30"
                    >
                    ...
                    <a
                        href="/assets/curriculo-isaque-maia.pdf"
                        download
                        className="inline-flex items-center gap-2 rounded-2xl border border-[rgba(255,107,53,0.4)] bg-[rgba(255,107,53,0.1)] px-6 py-3 text-sm font-semibold tracking-tight text-[#FF6B35] backdrop-blur-sm transition-all duration-300 hover:bg-[rgba(255,107,53,0.2)] hover:border-[rgba(255,107,53,0.7)] focus:outline-none focus:ring-2 focus:ring-[#FF6B35]/30"
                    >
                        <Download size={18} />
                        Baixar CV
                    </a>
            ...
            <div className="pointer-events-none absolute inset-x-0 -bottom-[60px] h-[120px] bg-gradient-to-b from-transparent to-[#0A0A0A] z-20" />
        </section>
    );
}
